/**
 * 权限标识管理页：搜索 + TanStack Table 分页列表 + 新增/编辑弹窗 + 删除
 * 由低代码生成器生成，可在此基础上精修
 */
...
export default function ButtonPage() {
  ...
  const listQuery = useQuery({
    queryKey: ['button-list', params],
    queryFn: () => ButtonApi.list(params),
  })
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {name: '', code: '', status: false},
  })

  function openCreate() {
    setEditing(null)
    form.reset({name: '', code: '', status: false})
    setDialogOpen(true)
  }

  function openEdit(row: ButtonRecord) {
    setEditing(row)
    form.reset({name: row.name ?? '', code: row.code ?? '', status: row.status ?? false })
    setDialogOpen(true)
  }

  const saveMutation = useMutation({
    mutationFn: (values: FormValues) =>
      editing ? ButtonApi.update(editing.id, values) : ButtonApi.create(values),
    onSuccess: () => {
      toast.success(editing ? '修改成功' : '新增成功')
      setDialogOpen(false)
      queryClient.invalidateQueries({ queryKey: ['button-list'] })
    },
    onError: (error: Error) => toast.error(error.message),
  })

  const deleteMutation = useMutation({
    mutationFn: (id: number) => ButtonApi.remove(id),
    onSuccess: () => {
      toast.success('删除成功')
      setDeleting(null)
      queryClient.invalidateQueries({ queryKey: ['button-list'] })
  ...
  return (
    <div className="space-y-4">
                  ...
                  <TableRow key={row.original.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Auth code="button:update">
                          <Button variant="ghost" size="sm" onClick={() => openEdit(row.original)}>
                            <Pencil className="size-4" />
                          </Button>
                        </Auth>
                        <Auth code="button:delete">
                          <Button
                            variant="ghost"
                            size="sm"
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? '编辑权限标识' : '新增权限标识'}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form
            ...
            >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>权限名称<span className="text-destructive">*</span></FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>权限值<span className="text-destructive">*</span></FormLabel>
                    <FormControl>

                      <Input placeholder="请输入权限值" {...field} />
